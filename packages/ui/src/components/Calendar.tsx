...
import { Button, buttonVariants } from "@pipu/ui/components";
...
function Nav({
  ...
  const isNextDisabled = (() => {
    if (navView === "years") {
      return (
        (startMonth &&
          dayjs(new Date(displayYears.to + 1, 0, 1)).diff(
            dayjs(startMonth),
            "days",
          ) < 0) ||
        (endMonth &&
  ...
  return (
    <div className={cn("grid grid-cols-4 gap-y-2", className)} {...props}>
      {Array.from(
        ...
        (_, i) => {
          const isBefore =
            dayjs(new Date(displayYears.from + i, 11, 31)).diff(
              dayjs(startMonth!),
              "days",
            ) < 0;

          const isAfter =
            dayjs(new Date(displayYears.from + i, 0, 0)).diff(
              dayjs(endMonth)!,
              "days",
            ) > 0;
