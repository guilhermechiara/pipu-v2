...
const HeaderDesktopLogo = () => {
  return (
    ...
    >
      <Image src={HeaderLogo} width={131} height={38} alt="Pipu logo" />
    </Link>
  );
};
...
const Header = ({ authenticated }: { authenticated: boolean }) => {
  return (
    <header
      className={`sticky top-0 z-50 flex items-center gap-6 h-(--header-height) shadow-header px-4 bg-primary`}
    >
      <HeaderDesktopLogo />
      {authenticated ?? <HeaderSearchInput />}
    </header>
  );
};
